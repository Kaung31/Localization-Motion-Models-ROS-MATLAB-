%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Task 1 - Closed-Loop Robot Controller using MATLAB and ROS
%
% Description:
% Implements a reactive obstacle avoidance controller using Lidar data.
% Commands velocity to explore the environment while avoiding collisions.
% Logs ground truth pose, Lidar ranges, velocity commands, and time steps.
%
% Author: Kaung Min Khant
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clear; clc;

global GL_robot_pose GL_ranges;

n = 6500;                      % Total iterations (~3 mins @ 30Hz)
sensor_max = 3.5;              % Maximum usable Lidar range (meters)

% Data storage arrays
robot_pose  = zeros(3, n);     % Ground truth pose: [x; y; theta]
ranges      = zeros(3, n);     % Lidar readings: [front; left; right]
vel_sent    = zeros(2, n);     % Velocity commands sent: [linear; angular]
time_log    = zeros(1, n);     % Execution time per loop

% Reset simulation
[resetclient, resetmsg] = rossvcclient('gazebo/reset_world');
resetclient.call(resetmsg);
pause(1);

% ROS setup
get_scan = rossubscriber("/scan", "sensor_msgs/LaserScan", @ROSRangeCallback, "DataFormat", "struct");
get_odom = rossubscriber("/odom", "nav_msgs/Odometry", @ROSPoseCallback, "DataFormat", "struct");
[velcmd, vel] = rospublisher("/cmd_vel", "geometry_msgs/Twist", "DataFormat", "struct");

% Wait for sensor callback
disp('Waiting for Lidar...');
while isempty(GL_ranges)
    pause(0.1);
end
disp('Lidar ready.');

% Control logic setup
turning = false;
turn_counter = 0;

% Main control loop
for i = 1:n
    tic;
    pause(0.03);  % Control rate ~30Hz

    % Ensure Lidar has valid data
    if isempty(GL_ranges)
        continue;
    end

    % Clamp noisy readings
    front = min(GL_ranges(1), sensor_max);
    left  = min(GL_ranges(2), sensor_max);
    right = min(GL_ranges(3), sensor_max);

    % Log current data
    ranges(:, i) = [front; left; right];
    robot_pose(:, i) = GL_robot_pose;

    % Obstacle avoidance logic    
    if turning
        vel.Linear.X = 0.05;
        vel.Angular.Z = vel.Angular.Z;
        turn_counter = turn_counter - 1;
        if turn_counter <= 0
            turning = false;
        end
    elseif front < 0.9
        % Obstacle detected
        if left > right
            vel.Angular.Z = 0.8;
        else
            vel.Angular.Z = -0.8;
        end
        vel.Linear.X = 0.05;
        turning = true;
        turn_counter = 20;
    else
        % Clear path ahead
        vel.Linear.X = 0.2;
        vel.Angular.Z = 0.0;
    end

    % Send command and log
    vel_sent(:, i) = [vel.Linear.X; vel.Angular.Z];
    send(velcmd, vel);
    time_log(i) = toc;

    % Print debug info every 100 iterations
    if mod(i, 100) == 0
        fprintf('[%04d] Front=%.2f | Left=%.2f | Right=%.2f\n', i, front, left, right);
    end
end

% Stop robot
vel.Linear.X = 0;
vel.Angular.Z = 0;
send(velcmd, vel);

%% Plotting 
figure(1);
plot(ranges(1,:), 'k'); hold on;
plot(ranges(2,:), 'r'); plot(ranges(3,:), 'b');
legend('Front', 'Left', 'Right');
title('Task 1: Lidar Readings');
xlabel('Time Step'); ylabel('Distance (m)');

%% Plot robot path
figure(2);
plot(robot_pose(1,:), robot_pose(2,:), 'b-', 'LineWidth', 2);
grid on; axis equal;
xlabel('X Position (m)');
ylabel('Y Position (m)');
title('Robot Ground Truth Path');

% mark start and end
hold on;
plot(robot_pose(1,1), robot_pose(2,1), 'go', 'MarkerSize', 10, 'DisplayName', 'Start');
plot(robot_pose(1,end), robot_pose(2,end), 'rx', 'MarkerSize', 10, 'LineWidth', 2, 'DisplayName', 'End');
legend('Path', 'Start', 'End');


%% Save Data 
% Save text files for external use
fileID = fopen('task1/odometry.dat','w');
fprintf(fileID, '%f %f %f\n', robot_pose);
fclose(fileID);

fileID = fopen('task1/ranges.dat','w');
fprintf(fileID, '%f %f %f\n', ranges);
fclose(fileID);

% Save MATLAB .mat files
save('task1/robot_pose.mat', 'robot_pose');
save('task1/vel_sent.mat', 'vel_sent', 'time_log');
save('task1/ranges.mat', 'ranges');

fprintf('Task 1 complete: data saved in task1 folder.\n');
